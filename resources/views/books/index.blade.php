...
<table class="w-full">
<tr class="border-b bg-gray-50"><th class="p-2 text-left">Reference</th><th>Subject</th><th>Title</th><th>Price</th><th>Student Ref</th><th></th></tr>
@foreach($items as $b)
<tr class="border-b">
<td class="p-2">{{ $b->reference }}</td>
<td>{{ $b->subject }}</td>
<td>{{ $b->title }}</td>
<td>£{{ number_format($b->price,2) }}</td>
<td class="text-sm">
  @if($b->student_reference)
...
<td class="text-right p-2">
  ...
  <form class="inline" method="POST" action="{{ route('books.destroy',$b) }}">@csrf @method('DELETE')<button class="text-red-600 ml-2">Delete</button></form>
</td>
</tr>
@endforeach
</table>
